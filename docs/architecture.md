...
- `LightController` reads time, tracks consecutive provider failures, logs safe-mode and output transitions, and applies state changes only when the simulated output must change.
...
- `ConsoleLightOutput` is an honest demo adapter. It stores its current state and does not claim to control hardware. Transition logging remains in the controller to avoid duplicate messages.
